      ...
      parseVarId(decl, kind) {
        ...
        if (kind === "using" || kind === "await using") {
          ...
          if (id2.type === "VoidPattern") {
            this.raise(Errors.UnexpectedVoidPattern, id2.loc.start);
          }
        }
        this.checkLVal(id2, {
...
// src/settings.ts
var PluginSettings = Type.Object({
...
function parse(sourceCode, config2 = DEFAULT_CONFIG) {
  const matches = [];
  const strategies = [
  ...
  for (const strategy of strategies) {
    try {
      const ast = strategy();
      if (ast) {
        traverseNode(ast, sourceCode, matches, config2);
        return matches;
      }
    } catch {
      continue;
...
function parseAsIs(sourceCode) {
  return (0, import_parser.parse)(sourceCode, {
    sourceType: "module",
...
function parseWithWrapper(sourceCode) {
  const wrappedCode = `<>${sourceCode.trim()}</>`;
  return (0, import_parser.parse)(wrappedCode, {
    sourceType: "module",
    allowImportExportEverywhere: true,
    allowReturnOutsideFunction: true,
    plugins: [
...
function traverseNode(node, sourceCode, matches, config2) {
  if (!node || typeof node !== "object") return;
  if (node.type === "CallExpression") {
    handleFunctionCall(node, sourceCode, matches, config2);
  }
  if (node.type === "JSXAttribute") {
    handleJSXAttribute(node, sourceCode, matches, config2);
  }
  for (const key in node) {
    const child = node[key];
    if (Array.isArray(child)) {
      child.forEach((item) => traverseNode(item, sourceCode, matches, config2));
    } else if (child && typeof child === "object") {
      traverseNode(child, sourceCode, matches, config2);
    }
  }
}
...
function handleFunctionCall(node, sourceCode, matches, config2) {
  const functionName = getFunctionName(node.callee);
  if (!functionName || !config2?.recognizedTfuncNames || !config2.recognizedTfuncNames.includes(functionName)) {
    return;
  }
...
function handleJSXAttribute(node, sourceCode, matches, config2) {
  if (node.name.type !== "JSXIdentifier") return;
  const attributeName = node.name.name;
  if (!attributeName || !config2.recognizedJSXAttributes || !config2.recognizedJSXAttributes.includes(attributeName)) {
    return;
  }
...
// src/ideExtension/config.ts
var ideExtensionConfigFactory = (settings) => ({
...
var marketplace_manifest_default = {
  $schema: "https://inlang.com/schema/marketplace-manifest",
  id: "plugin.minibits.inlangmatcher",
  displayName: {
...
// src/plugin.ts
var { displayName, description } = marketplace_manifest_default;
